...
**GitHub Pages**: https://lukechenug.github.io/picture-book-platform/
...
1. 点击导航栏的"LORA标注工具"按钮
2. 选择"批量编辑"模式
3. 上传多个TXT文件（按住Ctrl/Cmd多选）
4. 输入要添加的词组（如：`masterpiece, best quality`）
5. 选择添加位置（开头或结尾）
6. 选择连接方式（逗号+空格、空格、换行等）
7. 实时预览修改结果
8. 选择保存方式导出
...
### v1.3.0 (2025-10-27)
- ✨ **新增批量编辑功能**：统一给多个txt文件添加触发词
- ✅ 支持一次上传多个文件（按住Ctrl/Cmd多选）
- ✅ 灵活添加位置（开头或结尾）
- ✅ 多种连接方式（逗号+空格、空格、换行等）
- ✅ 实时预览原文和新文对比
- ✅ 双模式切换（文本拆分 / 批量编辑）
...
### Q: 如何给100个txt文件统一添加触发词？
A: 使用LORA工具的"批量编辑"模式，一次上传所有文件，输入触发词，选择添加位置和连接方式，即可批量处理。
